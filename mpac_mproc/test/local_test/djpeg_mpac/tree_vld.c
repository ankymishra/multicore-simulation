...
int load_huff_tables()
{
  ...
  while (size>0) {
    ...
    MaxDepth = 0;
    for (i=0; i<16; i++) {
      LeavesN = HTable[id][MAX_SIZE(hclass)-2*i-1] = mgetc();
    ...
    while (i<= MaxDepth) {
      ...
      for (k = 0; k<LeavesN; k++)
	if (!done) {
	  HTable[id][NextCellPt++] = mgetc() | GOOD_LEAF_FLAG;	
	  if (NextCellPt >= MAX_CELLS(hclass)) {
	    done = 1;
	    printf("\tWARNING:\tTruncating Table at depth %d\n", i+1);
	  }
	}
	else dummy = mgetc();	/* throw it away, just to keep file sync */
      size -= LeavesN;
...
unsigned char get_symbol(int select)
{
  ...
  while (HUFF_FLAG(HTable[select][cellPt]) == GOOD_NODE_FLAG)
    cellPt = get_one_bit() | (HUFF_VALUE(HTable[select][cellPt])<<1);

  switch (HUFF_FLAG(HTable[select][cellPt])) {
  case SPECIAL_FLAG:
    printf("%ld:\tERROR:\tFound forbidden Huffman symbol !\n", image_pos);
    printf("select=%d,cellPt=%d,(HUFF...)<<1=%d\n", select, cellPt,(HUFF_VALUE(HTable[select][cellPt])<<1));
    mt_halt();
    break;
  ...
  case BAD_LEAF_FLAG:
    ...
    printf("%ld:\tWARNING:\tFalling out of truncated tree !\n", image_pos);
    return 0;
    break;
