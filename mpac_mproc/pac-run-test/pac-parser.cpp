...
{
	for (int i = 0; i < PATTERN_DSPNUM; i++) {
		arg->core_bin[i] = (char *)malloc(sizeof(char) * 256);
		arg->core_tic[i] = (char *)malloc(sizeof(char) * 256);
	}
...
int pattern_stok(char *buf, struct pattern_arg *arg)
{
	...
	while (buf[i] != '\n') {
		...
		if (tmp_buf[j] == '#') {
			...
			tmp_buf[j] = '\0';
			if (core_index >= PATTERN_DSPNUM) {
				strcpy(arg->core_tic[core_index - PATTERN_DSPNUM], tmp_buf);
				core_index++;
			} else {
				strcpy(arg->core_bin[core_index], tmp_buf);
				core_index++;
			}
			j = 0;
		} else {
...
void iss_compare(FILE * fp)
{
	...
	list_for_each(pos, &(list_compare->list)) {
		dmem_read(((struct tic_head *)pos)->address, reinterpret_cast < unsigned char *>(&dmem_read_value), 
							sizeof(dmem_read_value));
...
void iss_setup()
{
	...
	list_for_each(pos, &(list_set->list)) {
		dmem_write(((struct tic_head *)pos)->address, reinterpret_cast < unsigned char *>(&(((struct tic_head *)pos)->value)), sizeof(((struct tic_head *)pos)->value));
		free(pos->prev);
	}
}
...
void pattern_sim_parser(char *filename)
{
	...
	multi_sim_arg.boot.cfu_mode = Get_Value(filename, "BOOT_OPTION", "CFU_MODE", 0);
