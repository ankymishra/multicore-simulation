...
SC_MODULE(SC_pac_if)
{
	...
	struct icache_group *icache_mem;
	...
	, multi_sim_arg(arg) {
		...
		icache_mem = new icache_group[l1_icache_size / l1_icache_line_size / GROUP_WAY];
		unsigned int gb = 0;
		for (i = 0; i < (l1_icache_size / l1_icache_line_size / GROUP_WAY); i++) {
			gb = GROUP_WAY;
 			while(gb--) {
				icache_mem[i].l1_icache_line[gb].insn = new unsigned char[l1_icache_line_size];
