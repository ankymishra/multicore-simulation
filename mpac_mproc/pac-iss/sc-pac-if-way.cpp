...
void SC_pac_if::l1_icache_read(unsigned int addr, unsigned char *buf, int len)
{
	...
	unsigned int gb = 0, max_lru_count = 0; 
	struct icache_replace ir_data_line1, ir_data_line2;
	unsigned int block_num = 0, hit = 0;
	unsigned int line1_length, line2_length;
	unsigned char *tmp_buf = buf;

	line1_length = l1_icache_line_size - (addr & (l1_icache_line_size - 1));
	line2_length = len - line1_length;
	...
	for (i = 0; i < len; i++) {
		...
		for (block_num = 0; block_num < GROUP_WAY; block_num++) {
			if ((icache_mem[iaddr.group].l1_icache_line[block_num].flag.invalid == 0)
				&& (icache_mem[iaddr.group].l1_icache_line[block_num].flag.tag == iaddr.tag)
				&& (icache_mem[iaddr.group].l1_icache_line[block_num].flag.line == iaddr.line)) {
//Hit				
				icache_mem[iaddr.group].l1_icache_line[block_num].flag.lru_count = 0;
				if (addr == tmp_addr) {
					memcpy(buf,
							&icache_mem[iaddr.group].l1_icache_line[block_num].insn[addr & (l1_icache_line_size - 1)],
							line1_length);
					...
					if (ir_data_line1.hit == 0) {
						...
						while(gb--) {
							...
							icache_mem[iaddr.group].l1_icache_line[gb].flag.lru_count++;
						}
						ir_data_line1.hit = 1;
					}
				} else if (addr == ((tmp_addr & ~(l1_icache_line_size - 1)) + l1_icache_line_size)){
					memcpy(buf,
							&icache_mem[iaddr.group].l1_icache_line[block_num].insn[addr & (l1_icache_line_size - 1)],
							line2_length);
...
//LINE 1
				addr_line1 = tmp_addr;
				iaddr_line1 = *(struct icache_addr *)&(addr_line1);

				for (block_num = 0; block_num < GROUP_WAY; block_num++) {
					if ((icache_mem[iaddr_line1.group].l1_icache_line[block_num].flag.invalid == 0)
						&& (icache_mem[iaddr_line1.group].l1_icache_line[block_num].flag.tag == iaddr_line1.tag)
						&& (icache_mem[iaddr_line1.group].l1_icache_line[block_num].flag.line == iaddr_line1.line)) {
				...
				if (ir_data_line1.hit == 0) {
					...
					while(gb--) {
						max_lru_count =
							max_lru_count >= icache_mem[iaddr_line1.group].l1_icache_line[gb].flag.lru_count
							? max_lru_count
							: icache_mem[iaddr_line1.group].l1_icache_line[gb].flag.lru_count;
					...
					icache_mem[iaddr_line1.group].l1_icache_line[ir_data_line1.block].flag.invalid = 0;
					icache_mem[iaddr_line1.group].l1_icache_line[ir_data_line1.block].flag.line = iaddr_line1.line;
					icache_mem[iaddr_line1.group].l1_icache_line[ir_data_line1.block].flag.tag = iaddr_line1.tag;
					icache_mem[iaddr_line1.group].l1_icache_line[ir_data_line1.block].flag.lru_count = 0;
					...
					ir_data_line1.hit = 1;
					memcpy(icache_mem[iaddr_line1.group].l1_icache_line[ir_data_line1.block].insn,
						(unsigned char *)base_ptr->buf,  l1_icache_line_size);
				}
				memcpy(tmp_buf,
					&icache_mem[iaddr_line1.group].l1_icache_line[ir_data_line1.block].insn[tmp_addr & (l1_icache_line_size - 1)],
					line1_length);
...
//LINE2
				...
				for (block_num = 0; block_num < GROUP_WAY; block_num++) {
					if ((icache_mem[iaddr_line2.group].l1_icache_line[block_num].flag.invalid == 0)
						&& (icache_mem[iaddr_line2.group].l1_icache_line[block_num].flag.tag == iaddr_line2.tag)
						&& (icache_mem[iaddr_line2.group].l1_icache_line[block_num].flag.line == iaddr_line2.line)) {
				...
				if (ir_data_line2.hit == 0) {
					...
					while(gb--) {
						max_lru_count =
							max_lru_count >= icache_mem[iaddr_line2.group].l1_icache_line[gb].flag.lru_count
							? max_lru_count
							: icache_mem[iaddr_line2.group].l1_icache_line[gb].flag.lru_count;
					...
					icache_mem[iaddr_line2.group].l1_icache_line[ir_data_line2.block].flag.invalid = 0;
					icache_mem[iaddr_line2.group].l1_icache_line[ir_data_line2.block].flag.line = iaddr_line2.line;
					icache_mem[iaddr_line2.group].l1_icache_line[ir_data_line2.block].flag.tag = iaddr_line2.tag;
					icache_mem[iaddr_line2.group].l1_icache_line[ir_data_line2.block].flag.lru_count = 0;
					...
					ir_data_line2.hit = 1;
					memcpy(icache_mem[iaddr_line2.group].l1_icache_line[ir_data_line2.block].insn,
						(unsigned char *)base_ptr->buf + l1_icache_line_size,  l1_icache_line_size);
				}

				memcpy(tmp_buf + line1_length,
					&icache_mem[iaddr_line2.group].l1_icache_line[ir_data_line2.block].insn[0],
					line2_length);
			} else {
//LINE 1
				addr_line1 = tmp_addr;
...
int SC_pac_if::icache_write(unsigned int addr, unsigned char *buf, int len)
{
	...
	unsigned int i, gb;
	for (i = 0; i < (l1_icache_size / l1_icache_line_size / GROUP_WAY); i++) {
		gb = GROUP_WAY;
 		while(gb--) {
			icache_mem[i].l1_icache_line[gb].flag.tag = 0;
			icache_mem[i].l1_icache_line[gb].flag.group = i;
			icache_mem[i].l1_icache_line[gb].flag.lru_count = 0;
			icache_mem[i].l1_icache_line[gb].flag.invalid = 1;
			icache_mem[i].l1_icache_line[gb].flag.line = 0;
		}
	}
