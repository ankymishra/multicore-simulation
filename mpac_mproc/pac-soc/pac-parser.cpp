...
struct list_head *list_set;
struct list_head *list_compare;
...
void soc_sim_setup(struct sim_arg *multi_sim_arg)
{
	...
	memset((void *)multi_sim_arg->boot.soc_shm_name, '\0', sizeof(multi_sim_arg->boot.soc_shm_name));

	multi_sim_arg->boot.soc_fifo_name = (char *)malloc(256 * sizeof(char));
	memset((void *)multi_sim_arg->boot.soc_fifo_name, '\0', sizeof(multi_sim_arg->boot.soc_fifo_name));
...
{
	free(multi_sim_arg->boot.soc_shm_name);
	free(multi_sim_arg->boot.soc_fifo_name);
	free(multi_sim_arg->boot.core_shm_name);
	free(multi_sim_arg->boot.m2_shm_name);
	free(multi_sim_arg->boot.ddr_shm_name);
	free(multi_sim_arg->boot.sysdma_shm_name);
	free(multi_sim_arg->boot.soc_fifo_name);
}
...
//SOC_SHM_NAME
	if (Get_String(filename, "BOOT_OPTION", "SOC_SHM_NAME") != NULL) {
		strcpy((char *)multi_sim_arg->boot.soc_shm_name, Get_String(filename, "BOOT_OPTION", "SOC_SHM_NAME"));
	}

	if (Get_String(filename, "BOOT_OPTION", "SOC_FIFO_NAME") != NULL) {
		strcpy((char *)multi_sim_arg->boot.soc_fifo_name, Get_String(filename, "BOOT_OPTION", "SOC_FIFO_NAME"));
	}
...
//M1
		...
		multi_sim_arg->pacdsp[i].m1_mem.dmem_m1_bank_size = Get_Value(filename, core_name, "M1_BANK_SIZE", 0);
		...
		multi_sim_arg->pacdsp[i].m1_mem.m1_rd_cont_delay = Get_Value(filename, core_name, "M1_MEM_RD_CONT_DELAY", 0);
		multi_sim_arg->pacdsp[i].m1_mem.m1_wr_cont_delay = Get_Value(filename, core_name, "M1_MEM_WR_CONT_DELAY", 0);
...
//D1
		multi_sim_arg->pacdsp[i].d1_cache.d1_cache_type = Get_Value(filename, core_name, "D1_CACHE_LINE_TYPE", 0);
		multi_sim_arg->pacdsp[i].d1_cache.d1_cache_size = Get_Value(filename, core_name, "D1_CACHE_SIZE", 0);
		multi_sim_arg->pacdsp[i].d1_cache.d1_cache_line_size = Get_Value(filename, core_name, "D1_CACHE_LINE_SIZE", 0);
		multi_sim_arg->pacdsp[i].d1_cache.d1_rd_delay = Get_Value(filename, core_name, "D1_CACHE_LINE_RD_DELAY", 0);
		multi_sim_arg->pacdsp[i].d1_cache.d1_wr_delay = Get_Value(filename, core_name, "D1_CACHE_LINE_WR_DELAY", 0);
...
//M2    
	...
	multi_sim_arg->m2_mem.dmem_m2_bank_size = Get_Value(filename, "M2_MEM", "BANK_SIZE", 0);
	...
	multi_sim_arg->m2_mem.m2_rd_cont_delay = Get_Value(filename, "M2_MEM", "RD_CONT_DELAY", 0);
	multi_sim_arg->m2_mem.m2_wr_cont_delay = Get_Value(filename, "M2_MEM", "WR_CONT_DELAY", 0);
...
//D2
	multi_sim_arg->d2_cache.d2_cache_type = Get_Value(filename, "D2_CACHE", "D2_CACHE_LINE_TYPE", 0);
	multi_sim_arg->d2_cache.d2_cache_size = Get_Value(filename, "D2_CACHE", "D2_CACHE_SIZE", 0);
	multi_sim_arg->d2_cache.d2_cache_line_size = Get_Value(filename, "D2_CACHE", "D2_CACHE_LINE_SIZE", 0);
	multi_sim_arg->d2_cache.d2_rd_delay = Get_Value(filename, "D2_CACHE", "D2_CACHE_LINE_RD_DELAY", 0);
	multi_sim_arg->d2_cache.d2_wr_delay = Get_Value(filename, "D2_CACHE", "D2_CACHE_LINE_WR_DELAY", 0);
...
//DDR
	...
	multi_sim_arg->ddr_mem.ddr_memory_bank_size = Get_Value(filename, "DDR", "BANK_SIZE", 0);
	...
	multi_sim_arg->ddr_mem.ddr_rd_cont_delay = Get_Value(filename, "DDR", "RD_CONT_DELAY", 0);
	multi_sim_arg->ddr_mem.ddr_wr_cont_delay = Get_Value(filename, "DDR", "WR_CONT_DELAY", 0);
